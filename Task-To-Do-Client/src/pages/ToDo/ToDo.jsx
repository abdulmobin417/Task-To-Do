    ...
    <div className="min-w-[346px] bg-[#F2F4F7] py-3 px-2 mb-auto ml-2">
      <div className="flex items-center justify-between ml-2">
        <div className="flex gap-2 items-center">
          <div className="bg-blue-500 h-6 w-5 rounded-l-full"></div>
          <div>
            <h2 className="text-gray-600 text-lg font-semibold">To Do</h2>
          </div>
        </div>
      ...
      <div className="mt-3 max-h-[800px] overflow-y-scroll task-scroll">
        {totalTask?.map((id) => (
          <SingleTask key={id} taskId={id} taskType={"ToDo"} />
        ))}
      </div>
    </div>
  );
